...
def monte_carlo_simulation(data, simulations=1000, days=30):
   ...
   for _ in range(simulations):
       price_series = [data['Close'].iloc[-1]]
       for i in range(days):
           price = price_series[-1] * (1 + np.random.standard_t(df=5, loc=mean_return, scale=volatility[i]))
           price_series.append(price)
       simulation_results.append(price_series)
   return simulation_results
...
def generate_recommendations(data, symbol=None):
   ...
   try:
       ...
       if 'Volume_Profile' in data.columns and data['Volume_Profile'].iloc[-1] is not None:
           vp = data['Volume_Profile']
           last_close = data['Close'].iloc[-1]
           max_vp_price = vp.idxmax()
           if abs(last_close - max_vp_price) < last_close * 0.02:  # Near high-volume node
