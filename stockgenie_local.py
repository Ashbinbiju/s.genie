...
def fetch_stock_data_cached(symbol, interval="5m", period="1d"):
    try:
        ...
        if data.empty:
            st.warning(f"⚠️ No data returned for {symbol} ({interval}) from Yahoo Finance.")
            return pd.DataFrame()
        return data
    except Exception as e:
        st.warning(f"⚠️ Error fetching data for {symbol} ({interval}): {str(e)}")
        return pd.DataFrame()
...
def detect_bearish_flag(data):
    ...
    if pole_drop < -0.02:  # 2%+ drop in 5 intervals (pole)
        flag_range = recent_data['High'].iloc[5:].max() - recent_data['Low'].iloc[5:].min()
        if flag_range < (recent_data['Close'].iloc[0] * 0.01):  # Tight consolidation (flag)
...
def analyze_stock(symbol):
    ...
    if data_5m.empty:
        st.warning("⚠️ No 5-minute data available to analyze.")
        return data_5m, {}, {}
    ...
    if missing_cols:
        st.warning(f"⚠️ Missing required columns in 5m data: {', '.join(missing_cols)}")
        return data_5m, {}, {}
...
def generate_recommendations(data, mtf_indicators, price_action, symbol=None):
    ...
    recommendations["Stop Loss"] = calculate_stop_loss(data, atr_multiplier=1.5)
    recommendations["Target"] = calculate_target(data, risk_reward_ratio=2)
